...
<p align="center">
  <img src="./screenshot.png" alt="Slowcloud 主题白天模式截图" width="720">
</p>
<p align="center">
  <img src="./screenshot-black.png" alt="Slowcloud 主题黑夜模式截图" width="720">
</p>
...
- SEO 能力增强，支持为文章单独设置 SEO 标题、SEO 描述、Canonical 地址和 Noindex。
- 自动输出 description、robots、Open Graph、Twitter Card 和 JSON-LD 结构化数据。
- 新增 Sitemap 与 Robots，主题可直接响应 `sitemap.xml` 和 `robots.txt`，Sitemap 会包含首页、文章、独立页面以及文章海报图片信息。
- 优化访问统计策略，主题自身即可记录 PV、UV、IP 和最近访问记录，`SlowcloudStatistics` 插件主要作为后台统计查看面板使用。
- 统计过滤更细，排除登录管理员、HEAD 请求、非 HTML 请求、预加载请求和常见爬虫 UA；被排除的访问可保留在最近访问记录中，但不会增加 PV、UV 和文章浏览量。
- 增加文章目录开关，默认关闭；支持文章海报 Alt 文本，并为有海报的文章提供默认海报、横向媒体和沉浸封面三种列表展示形式。
- 文章正文图片和文章海报支持点击放大；站内跳转时显示顶部加载条，并会根据 Header 是否在可视范围内自动调整位置。
- 支持自定义入口和社交平台配置，可在后台添加多条名称、SVG 图标和跳转地址，适配图标库没有覆盖的场景。
- 优化标签云和文章详情标签展示，统一添加 `#`，标签云展示文章数量；代码块支持复制按钮。
- 补齐主题静态资源版本号，优化编辑器预览、文章目录预览、代码块插入、自定义字段草稿保存。
